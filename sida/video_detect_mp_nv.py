...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        shape = img.shape[2:4]  # current shape [height, width]
        ...
        if shape[::-1] != new_unpad:  # resize
            ...
            # img = F.interpolate(img.to(dtype=torch.float32), (new_unpad[1], new_unpad[0]), mode='bilinear', align_corners=False).to(dtype=torch.uint8)
            img = F.interpolate(img, (new_unpad[1], new_unpad[0]), mode='nearest')
        top, bottom = int(round(dh - 0.1)) if self.center else 0, int(round(dh + 0.1))
        left, right = int(round(dw - 0.1)) if self.center else 0, int(round(dw + 0.1))
        img = F.pad(img, (left, right, top, bottom), "constant", 114) # add border
...
def video_detect_run(file_queue, out_queue, pid, args):
    try:
        gpu_id = (pid % args.gpu_count) + args.gpu_id_offset
        ...
        class Detection:
            def __init__(self, config, checkpoint=None, device='0'):
                ...
                with torch.no_grad():
                    self.model(
                        torch.zeros(1, 3, *imgsz).to(self.device).type_as(next(self.model.parameters())))  # run once
        ...
        def pre_batch_image(out_queue, file_name, batch_index, batch_images, batch_frameids, batch_w_hs):
            ...
            if image_count > 0:
                ...
                input_images = torch.concatenate(batch_images, 0)
        ...
        def tensor_display(img_tensor, name='tensor'):
            img_tensor.resize_(3, img_tensor.shape[-2], img_tensor.shape[-1])
            image = cv2.cvtColor(img_tensor.permute(1, 2, 0).cpu().numpy(), cv2.COLOR_RGB2BGR)
        ...
        while True:
            ...
            cap = nvc.PyNvDecoder(video_path, gpu_id)
            total_frame_count = int(cap.Numframes())
            ...
            batch_index = 0
            while True:
                src_surface = cap.DecodeSingleSurface()
                ...
                #     surf_plane.ElemSize(),
                # )
                # tensor_display(img_tensor)
                img_tensor.resize_(1, 3, h, w)

                image = detect_trans(img_tensor)
                # tensor_display(image)
                batch_images.append(image)
                batch_frameids.append(frame_count)
                batch_w_hs.append([w, h])
...
def video_detect_mp(args):
    ...
    for i in range(args.gpu_count):
        workers.append(multiprocessing.Process(target=video_detect_run, args=(file_queue, out_queue, i, args)))
